...
def count_occupied_dir(grid, x, y, direction_x, direction_y):
    res = 0
    cx = x
    cy = y
    ...
    # debug block
    if y == 8 and x == 0:
        print("X {} Y {} DIR:{}/{} - ".format(x, y, direction_x, direction_y), end="")
    ...
    while True:
        ...
        if char == "#":
            res = 1  # occupied in this direction
            break
        elif char == "L":
            res = 0  # unoccupied in this direction
            break
        # else "." ; continue
...
def count_adjacent_occupied(grid, x, y):
    ...
    res += count_occupied_dir(grid, x, y, -1, -1)
    res += count_occupied_dir(grid, x, y,  0, -1)
    res += count_occupied_dir(grid, x, y,  1, -1)
    res += count_occupied_dir(grid, x, y, -1,  0)
    ...
    for j in range(len(original)):
        for i in range(len(original[0])):
            ...
            #rule2
                ...
                res += 1
                if count_adjacent_occupied(original, i, j) >= 5:
                    grid[j][i] = "L"
                    res -= 1
    return res
