...
# Programmatic mode
def run_seq(commands=None, tee=False, interactive_mode=False):
    if commands is None or len(commands) == 0:
        memory = init_memory(values)
        res = run_code(mem=memory, input_values=list(), interactive_mode=interactive_mode, stop_on_input=True, tee=tee, reset=True)
    else:
        memory = init_memory(values)
        char_seq = list()
        for cmd in commands:
        ...
        res = run_code(mem=memory, input_values=char_seq, interactive_mode=interactive_mode, stop_on_input=False, tee=tee, reset=True)
    #print(">>>{}<<<".format(res))
    return res
...
def explore(path):
    ...
    if "== " not in description:
        print("Unexpected room description from path {}\n>>>{}<<<".format(path, description))
        return
...
#     print("  doors: {}".format(room["doors"]))
#     print("  items: {}".format(room["items"]))
#     print("  path to get there: {}".format(room["path"]))
...
def reverse_path(path : list):
    ...
    if path is not None and len(path) > 0:
        ...
        rev.reverse()
        for d in rev:
            res.append(reverse_direction(d))
    return res


# Find where items are
all_objects = dict()
for room in rooms.values():
    if room["items"]:
        for item in room["items"]:
            all_objects[item] = room["path"]

# print()
# for key in all_objects.keys():
#     val = all_objects[key]
#     print("path to object [{}] (from hull breach) is {}".format(key, val))
#     print("reverse path to hull breach from {} is {} ".format(key, reverse_path(val)))


def path_to_object(object_name):
    if object_name not in all_objects.keys():
        print("** unknown object {} **".format(object_name))
        return list()
    else:
        return all_objects[object_name]


def path_to_room(room_name):
    res = None
...
print()
print("path to secured room is {}".format(path_to_room("Pressure-Sensitive Floor")))
print("path to sec checkpoint room is {}".format(path_to_room("Security Checkpoint")))

def go_take_and_back(object_name):
    act = list()
    act.extend(path_to_object(object_name))
    act.append("take " + object_name)
    act.extend(reverse_path(path_to_object(object_name)))
    return act
...
# Try all combination (of non-lethal objects)
# print()
# nb_combi = 0
# for r in range(1, len(useable_objects)+1):
#     combinations = list(itertools.combinations(useable_objects, r))
#     #print(list(combinations)[0:10])
#     for combi in combinations:
#         nb_combi += 1
#         print("Trying {}-th combination {}".format(nb_combi, combi))
#         actions = list()
#         for o_name in combi:
#             actions.extend(go_take_and_back(o_name))
#         actions.extend(path_to_room("Pressure-Sensitive Floor"))
#         memory = init_memory(values)
#         output = run_seq(commands=actions, interactive_mode=False, tee=False)
#         if "Alert!" in output:
#             print("  this combination does trigger ALERT ".format(o_name))
#         else:
#             print("* * YEEEEEEEEEES this combination does NOT trigger alert".format(o_name))
#             #print("object {} does NOT trigger alert : {}".format(o_name, output))


# Trying 152-th combination ('fixed point', 'spool of cat6', 'shell', 'candy cane')
# EXIT PROGRAM
# * * YEEEEEEEEEES this combination does NOT trigger alert


# Macro mode / plus Interactive mode (if wanted)
actions = list()
actions.extend(go_take_and_back("fixed point"))
actions.extend(go_take_and_back("spool of cat6"))
actions.extend(go_take_and_back("shell"))
actions.extend(go_take_and_back("candy cane"))
actions.extend(path_to_room("Pressure-Sensitive Floor"))
#actions.extend(path_to_room("Security Checkpoint"))
# print(actions)
memory = init_memory(values)
output = run_seq(commands=actions, interactive_mode=True, tee=True)
if "Alert!" in output:
    print("ALERT FOUND")
else:
