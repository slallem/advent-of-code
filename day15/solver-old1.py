...
for ii in range(len(snums)-1):
    snum = snums[ii]
    turn += 1
    #said.append(int(snum))
    last_indexes[int(snum)] = turn
    print("turn {} last said {}".format(turn, snum))

print("--{}".format(len(said)))

#last_said = said[len(said)-1]
#said.pop()

last_said = int(snums[-1])
#del last_indexes[last_said]

#print("--{}".format(len(said)))

while True:
    turn += 1
    #is_new = last_said not in said

    #lasti = last_index_of(said, last_said)
    lasti: int = -1
    if last_said in last_indexes.keys():
        lasti = last_indexes[last_said]
    ...
    if lasti < 0:
        ...
        say = turn - lasti - 1
    last_said = say
    print("turn {} says {}".format(turn, last_said))
    #if turn == 30000000:
    #if turn >= 2020:
    if turn >= 10:
        break
