...
def custom_eval(expr: str, verbose: bool=False):
    ...
    for i in range(0, len(chars)):
        ...
        if char in "0123456789":
            sval += char
            if i >= (len(chars)-1) or chars[i+1] not in "0123456789":
                if ope == "+":
                    res += int(sval)
                    if verbose:
                        print("= plus {} donne {}".format(sval, res))
                elif ope == "*":
                    res *= int(sval)
                    if verbose:
                        print("= fois {} donne {}".format(sval, res))
                else:
                    raise Exception("Operateur inattendu {}".format(char))
            else:
                if verbose:
                    print("= concat now sval is {}".format(sval))
        elif char in "+*":
            if verbose:
                print("= ope {}".format(char))
            sval = ""
            ope = char
        else:
            raise Exception("Symbole inattendu {}".format(char))
...
# print(eval_ligne(expression="((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2", verbose=True))
# exit(99)
